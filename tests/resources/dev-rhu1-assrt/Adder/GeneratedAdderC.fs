...
type CallbacksC = {
    ...
    [<Refined("(st: State12) -> (f: int) -> unit")>] state12OnreceiveRES : State12 -> int -> unit
...
let run (callbacks : CallbacksC) (comms : Communications) : Async<unit> =
        ...
        async {
            ...
            match label with
                | State10Choice.BYE ->
                    ...
                    let _dummy = callbacks.state10OnsendBYE st
                ...
                | State10Choice.ADD ->
                    ...
                    let w = callbacks.state10OnsendADD st
                    do! comms.send_int S w
                    let st : State11 = {
                        u = st.u;
                        w = w;
                    }
    ...
    and runState11 (st: State11) : Async<unit> =
        async {
            do! comms.send_string S "ADD"
            let v = callbacks.state11OnsendADD st
            do! comms.send_int S v
            let st : State12 = {
                u = st.u;
                w = st.w;
                v = v;
